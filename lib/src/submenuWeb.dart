...
class _SubmenuState extends State<Submenu> with SingleTickerProviderStateMixin {
  static const double _defaultSubmenuHeight = 32.0;
  ...
  void _showSubmenu({ bool immediately = false }) {
    _hideTimer?.cancel();
    _hideTimer = null;
    if (immediately) {
    ...
    _showTimer ??= Timer(waitDuration, ensureSubmenuVisible);
  }

  bool ensureSubmenuVisible() {
    _showTimer?.cancel();
    _showTimer = null;
    if (_entry != null) {
  ...
  void _handleLongPress() {
    ...
    final bool tooltipCreated = ensureSubmenuVisible();
    if (tooltipCreated)
  ...
  Widget build(BuildContext context) {
    ...
    height = widget.height ?? tooltipTheme.height ?? _defaultSubmenuHeight;
    ...
    Widget result = GestureDetector(
      ...
      child: Semantics(
        label: excludeFromSemantics ? null : "00000",
        child: widget.child,
      ),
    );
    ...
    if (_mouseIsConnected) {
      result = MouseRegion(
        onEnter: (PointerEnterEvent event) => _showSubmenu(),
        onExit: (PointerExitEvent event) => _hideSubmenu(),
        child: result,
      );
    }
